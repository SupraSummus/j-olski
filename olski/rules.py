...
``UNCALIBRATED`` until somebody takes the measurement its check calls for, and
the shapes such a measurement comes in are :mod:`olski.calibration`, one module
below this one, because a calibration is no more a rule than it is a check.
...
from olski.calibration import UNCALIBRATED, Calibration, Uncalibrated
from olski.checks import ParamError, get_check
...
class Rule:
    ...
    #: What has been measured about this rule, in one of the shapes
    #: :mod:`olski.calibration` holds: the default until the milestone 1 harness
    #: replaces it with whichever shape the rule's check calls for.
    ...
    def __post_init__(self) -> None:
        ...
        owed = check.calibrated_by
        if not isinstance(self.calibration, (Uncalibrated, owed)):
            raise RuleError(
                f"{where}: check {self.check!r} calls for {owed.__name__} "
                f"or UNCALIBRATED, not {self.calibration!r}"
            )
