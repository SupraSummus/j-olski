...
Za flagą ``--dzierżawcze`` czeka drugi wykrywacz.
Reguła wyżej pyta, na którą z dwóch rzeczy zaimek wskazuje,
a ten pyta o rzecz, której zaimek dzierżawczy wskazać nie może
...
def zaimki_niezwrotne(czytania: Sequence[Node]) -> tuple[Niezwrotny, ...]:
    ...
    **Rzecz, pod którą zaimek stoi, kandydatem nie jest**, bo posiadacza nie ma
    ...
    Gdzie ta granica biegnie, rozstrzyga :func:`_głowy`.
    ...
    **Zdanie podrzędne ma własny podmiot**, którego to zejście nie widzi
    ...
    Ceną jest zdanie wieloznaczne, w którym rzecz zgodna stoi przed zaimkiem,
    a jedno z czytań stawia ją nad nim: w `Czy lekarz może leczyć pacjenta wbrew
    jego woli?` pacjent kandydatem nie jest w tym czytaniu, w którym `wbrew jego
    woli` dochodzi właśnie do niego, więc zostaje sam `lekarz` i zgłoszenie pada
    (``próba/nkjp-sądy.txt``).
    """
    znalezione: dict[tuple[str, str, str], None] = {}
    for czytanie in czytania:
        #  Czytanie bez zaimka dzierżawczego wychodzi stąd po jednym przejściu po
        #  węzłach, bo takich zdań jest większość, a listy niżej kosztują drugie.
        dzierżawcze = list(_dzierżawcze(czytanie))
        if not dzierżawcze:
            continue
        for zakres in zakresy(czytanie, DEKLARACJA.składowe):
            #  Składowe bez podmiotu — rozkaz, orzeczenie bezosobowe — nie ma czego
            #  zaimkowi zakazać, bo `swój` nie ma tam kogo podjąć.
            podmioty = w_zakresie(czytanie, DEKLARACJA.obsada.podmiot, DEKLARACJA.podrzędne, zakres)
            if len(podmioty) != 1:
                continue
            podmiot = podmioty[0].liść_głowy()
            for zaimek, rzecz in dzierżawcze:
                if (
                    _w_zakresie(zaimek, zakres)
                    and not _w_podmiocie(zaimek, podmioty[0])
                    and _tylko_podmiot(zaimek, podmiot, czytanie, zakres)
                ):
...
def _dzierżawcze(czytanie: Node) -> Iterator[tuple[Leaf, Leaf]]:
    """Zaimki dzierżawcze tego czytania wraz z rzeczą, którą każdy określa.
    ...
    for węzeł in _węzły(czytanie, DEKLARACJA.podrzędne):
        for numer, dziecko in enumerate(węzeł.children):
            if numer == węzeł.głowa or not isinstance(dziecko, Leaf):
                continue
            if any(r.tag.pos == ZAIMEK for r in dziecko.odczytania):
