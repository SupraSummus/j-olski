...
def test_plik_krótszy_od_wycinka_wchodzi_w_całości(korpus):
    ...
    for wycinek in wycinki(korpus):
        for kawałek in wycinek.kawałki:
            plik = korpus / kawałek.plik
            if plik.name != "txt_9.txt":
                zdań = len(plik.read_text(encoding="utf-8").splitlines())
                assert (kawałek.od, kawałek.do) == (1, zdań)


def test_wycinek_z_rejestru_nie_wraca(korpus):
    wszystkie = wycinki(korpus)
    assert następny(wszystkie, {wszystkie[0].nazwa}) == wszystkie[1]
