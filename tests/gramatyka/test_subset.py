...
Która produkcja zdanie wpuszcza, pyta plik swojej warstwy w tym katalogu,
a warstwy są te same, po których rejestr konstrukcji dzieli swoje pliki
(docs/konstrukcje-gramatyczne/README.md).

Warstwę pod gramatyką sprawdza ``tests/warstwa-leksykalna/test_segmentacja.py``,
a odpowiedzi, które werdykt dokłada nad rozbiorem —
fragment, niedomknięcie, zatrzymanie — sprawdza ``tests/werdykt/test_werdykt.py``.
Formalizm, którym podzbiór jest napisany, sprawdza ``tests/gramatyka/test_gramatyka.py``,
a to, co z lasu wychodzi nad zdaniem wieloznacznym, ``tests/parser/test_las.py``.
"""
