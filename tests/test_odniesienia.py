...
def test_zaimek_dzierżawczy_zgodny_z_samym_podmiotem_jest_zgłoszeniem():
    """Jedyne potwierdzone trafienie tej reguły (``próba/nkjp-sądy.txt``).

    Kandydatem na posiadacza jest tu sam podmiot, a jest jeden dlatego, że rzecz
    z zaimkiem pod sobą kandydata nie wydaje: bez tego zawężenia `omawiania`
    i `stosunku` dokładają dwóch i zgłoszenie schodzi.
    """
...
def test_zgłoszenie_o_zaimku_dzierżawczym_nie_jest_znaleziskiem():
    #  Reguła czeka na awans, a poza kod wyjścia trzyma ją własna nazwa, którą
    #  trzeba było wypisać poza `ZNALEZISKA` osobno od nazwy spod `w_zdaniu`.
